...
func curlHandler(w http.ResponseWriter, r *http.Request) {
	loops := 6
	for i := 0; i < len(frames)*loops; i++ {
		color := 31 + i%6
		fmt.Fprintf(w, "\033[2J\033[H")
		fmt.Fprintf(w, "\033[%dm", color)
		fmt.Fprintf(w, frames[i%len(frames)][1:])
		w.(http.Flusher).Flush()
		fmt.Fprintf(w, "\033[39m")
		time.Sleep(50 * time.Millisecond)
	}
	fmt.Fprintf(w, "\033[2J\033[H")
	fmt.Fprintf(w, "no resume here yet, but thanks for checking me out!\n")
}
